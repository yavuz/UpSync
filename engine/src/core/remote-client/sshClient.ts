  ...
  private async _connectSSHClient(
    ...
    if (option.passphrase === true) {
      option.passphrase = await config.askForPasswd(
        ...
        `passphrase:${option.username ?? ''}@${option.host ?? ''}`
      );
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      client
        ...
        // Upstream hatası: `this.end()` şeklindeydi - listener yerine
        // dönüş değeri (undefined) kaydediliyor ve end() bağlantı kurulurken
        // hemen çağrılıyordu. Node 22 undefined listener'ı reddediyor.
        .on('close', () => this.end())
        .on('end', () => this.end())
        .connect({
